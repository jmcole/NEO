...
class NearEarthObject:
    ...
    def __init__(self, **info):
        ...
        :param info: A dictionary of excess keyword arguments supplied to the
         constructor.
        """
        # Create NEO object structure add fullname to required fields
        self.designation = info["pdes"]
        self.name = info["name"] if info["name"] else None
        self.diameter = float(
            info["diameter"]) if info["diameter"] else float("nan")
        self.hazardous = True if info["pha"].capitalize() == ("Y") else False
        self.approaches = []
        self.fullname
...
class CloseApproach:
    ...
    def __init__(self, **info):
        ...
        constructor.
        """
        # Create Close Approach object structure
        self._designation = info["des"]
        self.time = cd_to_datetime(info["cd"])
        self.distance = float(info["dist"]) if info["dist"] else float("nan")
        self.velocity = float(info["v_rel"]) if info["v_rel"] else float("nan")
        self.neo = None
    ...
    def __str__(self):
        ...
        return (
            f"On {self.time_str}, "
            f"{self.neo.fullname} "
            f"approached Earth at a distance of {self.distance:.2f}au "
